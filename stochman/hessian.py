...
class HessianCalculator(ABC, nn.Module):
    def __init__(
        ...
        assert loss_func in ("mse", 
                             "cross_entropy_binary", "cross_entropy_multiclass", 
                             "contrastive_full", "contrastive_pos", "contrastive_fix", 
                             "arccos_full", "arccos_pos")
    ...
    def compute_loss(self, x, target, nnj_module, tuple_indices = None):
        ...
        elif "arccos" in self.loss_func:
            return self.compute_arccos(x, nnj_module, tuple_indices) #arccos loss does not have targets
    ...
    def compute_hessian(self, x, nnj_module, tuple_indices = None):
        ...
        with torch.no_grad():
            if self.loss_func == "mse":
                ...
                return self.compute_arccos_hessian(x, nnj_module, tuple_indices)
    ...
    def compute_arccos(self, x, nnj_module, tuple_indices):
        '''
    ...
    def compute_arccos_hessian(self, x, nnj_module, tuple_indices):
        ...
        if self.loss_func == "arccos_full" or self.loss_func == "arccos_pos":
            ...
            # backpropagate through the network
            pos = nnj_module._jTmjp_batch2(x[ap], x[p], None, None, H,
                                           wrt = self.wrt,
                                           to_diag = self.shape=="diagonal", from_diag = False, 
                                           diag_backprop = self.speed=="fast")
            if self.shape=="diagonal":
                if self.wrt == "weight":
                    pos = [matrixes[0] - 2 * matrixes[1] + matrixes[2] for matrixes in pos]
            ...
            # backpropagate through the network
            neg = nnj_module._jTmjp_batch2(x[an], x[n], None, None, H,
                                           wrt = self.wrt,
                                           to_diag = self.shape=="diagonal", from_diag = False, 
                                           diag_backprop = self.speed=="fast")
            if self.shape=="diagonal":
